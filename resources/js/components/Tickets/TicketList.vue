<template>
    <div class="p-4">
      ...
      <div v-if="!loading && !error && tickets.length === 0" class="p-4 text-center text-gray-500 bg-white rounded-lg shadow">
        {{ isAdmin ? 'No tickets found in the system.' : 'You haven\'t created any tickets yet.' }}
      </div>
          ...
          <router-link :to="{ name: 'tickets.show', params: { id: ticket.id } }" class="block">
            ...
            <div class="flex items-center justify-between mt-4 text-xs text-gray-500">
              <div class="flex items-center space-x-4">
                ...
                <span v-if="isAdmin && ticket.user">By: {{ ticket.user.name }}</span>
              </div>
  ...
  const pageTitle = computed(() => {
      return isAdmin.value ? 'All Tickets' : 'My Tickets';
  });
